...
/// In-memory form of a `.darkroom` bundle (design D3): the document JSON, every
/// referenced image's encoded bytes (PNG), and an optional pre-rendered thumbnail.
/// This layer stores bytes — it never decodes pixels (that's DarkroomRender's job),
...
/// Reads/writes `.darkroom` bundles on disk.
///
...
public enum DocumentBundleIO {
    public static let pathExtension = "darkroom"
