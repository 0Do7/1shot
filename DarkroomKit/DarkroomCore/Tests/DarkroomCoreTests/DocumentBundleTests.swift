...
private func temporaryDirectory() throws -> URL {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("darkroom-bundle-tests-\(UUID().uuidString)")
...
@Test func bundle_writeReadRoundTrip() throws {
    ...
    let url = dir.appendingPathComponent("sample.darkroom")
...
@Test func bundle_writeRefusesWhenReferencedImageMissing() throws {
    ...
    #expect(throws: DocumentBundleError.missingImage("image-2.png")) {
        try DocumentBundleIO.write(bundle, to: dir.appendingPathComponent("broken.darkroom"))
    }
}
...
@Test func bundle_readRefusesTraversalImageNames() throws {
    ...
    #expect(throws: DocumentBundleError.invalidImageName("../escape.png")) {
        try DocumentBundleIO.write(bundle, to: dir.appendingPathComponent("hostile.darkroom"))
    }
}
...
@Test func bundle_missingDocumentJSON_isExplicit() throws {
    let dir = try temporaryDirectory()
    defer { try? FileManager.default.removeItem(at: dir) }
    let url = dir.appendingPathComponent("empty.darkroom")
    try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
...
@Test func goldenV1Bundle_opens() throws {
    ...
    let bundle = try DocumentBundleIO.read(from: fixtures.appendingPathComponent("golden-v1.darkroom"))
